...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void scheduleCheckInOutNotifications(Booking booking) {
        ...
        Instant checkInReminder = convertToInstantAndAddHours(booking.getCheckIn(), -24);
        taskScheduler.schedule(
                () -> createCheckInReminder(booking),
                checkInReminder
        );

        // Schedule check-out reminder (2 hours before)
        Instant checkOutReminder = convertToInstantAndAddHours(booking.getCheckOut(), -2);
        taskScheduler.schedule(
                () -> createCheckOutReminder(booking),
                checkOutReminder
        );
    }
    ...
    private Instant convertToInstantAndAddHours(LocalDateTime localDateTime, int hours) {
